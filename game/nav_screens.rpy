...
init -1:
    ...
    image tooltip_inventory_prelim=LiveComposite((liveComWidth, liveComHeight), (titleX,titleY), ImageReference(Text("Autopsy Report", style="tips_top")), (descriptionX,descriptionY), Text("The victim has a stab wound in his back and bruises all over his body. Exact cause of death unknown.", style="tips_bottom"))
...
screen inventory_screen():
    ...
    for item in sorted_items:
        ...
        $ i += 1
            ...
            #imagebutton idle "gui/inv_arrow.png" action [SetVariable('inv_page', next_inv_page), Show("present_evidence_screen")] xpos .75 ypos .9 xpadding 40 ypadding 20
...
screen present_evidence_screen():
    ...
    for item in sorted_items:
        ...
        $ i += 1
        #if len(inventory.items)>9:
            #imagebutton idle "gui/inv_arrow.png" action [SetVariable('inv_page', next_inv_page), Show("present_evidence_screen")] xpos .75 ypos .9
...
screen gui_check (check_pic="", my_check_xpos=42, my_check_ypos=100):
    add check_pic xpos 30 ypos 219 zoom 0.86
...
screen testi():
    ...
    if store.testiright <> None:
        imagebutton auto "assets/menu/testi_arrow_%s.png" action Jump(store.testiright) xpos 0.18 ypos 0.58
    hbox align (1.0,0) spacing 10:
